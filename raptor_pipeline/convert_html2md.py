    ...
    try:
        ...
        if downloaded:
            ...
            output_file_path = os.path.join(
                output_path,
                os.path.splitext(os.path.basename(html_file_path))[0] + ".md",
            )
...
def process_directory(html_dir, output_dir, num_processes=None):
    ...
    for root, _, files in os.walk(html_dir):
        total_files += len(
            [f for f in files if f.endswith(".html") or f.endswith(".htm")]
        )
    ...
    with tqdm(total=total_files, desc="Converting HTML to Markdown") as pbar:
        ...
        for root, _, files in os.walk(html_dir):
            ...
            with multiprocessing.Pool(processes=num_processes) as pool:
                ...
                for file_path, success, error_msg in pool.imap_unordered(
                    process_func, html_files
                ):
                    pbar.update(1)
                    if not success:
...
MARKDOWN_DIR = (
    "markdown_output"  # Replace with the desired output directory for Markdown files
)
